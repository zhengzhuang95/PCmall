...
! function(a) {
	...
	var b = MOGU.Trade,
		...
		d = ['<label class="lableh" for="">省：<span class="need">*</span></label>', '<div class="inputbox">', '<select name="" class="province">', "</select>", "</div>", '<label for="">市：</label>', '<div class="inputbox">', '<select data-type="*" data-errormsg="请选择地址" data-inputbox="lastinput" class="town">','<option value="">请选择省份</option>','<option value="">请选择省份</option>','</select>', "</div>", '<label for="">区：</label>', '<div class="inputbox lastinput">', '<select name="" class="area">','<option value="">请选择省份</option>', "</select>", "</div>"].join("");
	b.selectAdress = function(b, d) {
		...
		renderEl: function() {
			var a = this;
			"" !== a.container.html && a.container.append(a.tempel), a.province = a.container.find(".province"), a.town = a.container.find(".town"), a.area = a.container.find(".area")
		},
	...
	function(t) {
		...
		e.prototype.init = function(e, s, r) {
			...
			for(var a = function(t, e) {
					t.removeClass("error").val(e), t.next().removeClass("notice").html(t.attr("data-normal"))
			...
			n.level = s, a && (n.parentId = a), t.ajax({
				...
				success: function(t) {
					if(t && t.status && 1001 === t.status.code) {
						...
						e.html(a.join("")), "function" == typeof o && o()
			...
			e.$pop.find(".J_form").on("blur", function() {
				...
				c ? (a.removeClass("notice").html(l), s.removeClass("error")) : (a.addClass("notice").html(i), s.addClass("error"))
							...
							s && s.status && 1001 === s.status.code ? (a.tn = s.result.tn, a.tv = s.result.tv, t.ajax({
								...
								success: function(s) {
									s && s.status && 1001 === s.status.code ? (a.addressId = s.result.addressId, e.$pop.find(".J_select").each(function() {
										var e = t(this),
											s = e.attr("name");
										a[s] = e.find("option").filter(":selected").html()
...
! function() {
	...
	t.click(function(e) {
		...
		return MOGU.editAddressBox(s, function() {
			...
			address: {
				...
				postcode: d.find(".zcode").html(),
				street: d.find(".addr").attr("data-street"),
				name: d.find(".name").html(),
				mobile: d.find(".mobile").html(),
	...
	a.on("click", ".save", function() {
		...
		var e = $(this);
		e.hasClass("disable") || (e.addClass("disable"), l.val(c.html().replace(/<br>/gim, "\n")), e.removeClass("disable"), h("save"))
		...
		t.each(function() {
			...
			new s({
				...
				callback: function() {
					...
					a.find(".order_time").remove(), a.find(".count_price").html('<span class="pay_btn closed">订单取消</span>')
				}
			})
		})
	}
